...
12. **The ALB controller's Pod Identity binding has to exist before its pod does (fixed in [#21](https://github.com/aws-samples/sample-litellm-bedrock-gateway-on-eks/pull/21); was [#19](https://github.com/aws-samples/sample-litellm-bedrock-gateway-on-eks/issues/19)):** EKS injects the Pod Identity env vars (`AWS_CONTAINER_CREDENTIALS_FULL_URI`, `AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE`) through a **mutating webhook at pod-creation time**. Create the association afterwards and the already-running pod is never given credentials — nothing back-fills them. The controller then logs `NoCredentialProviders: no valid providers in chain`, the Ingress `ADDRESS` stays empty and **no ALB is ever created**, while the pod itself is `Running` with a green liveness probe. `kubectl port-forward` smoke tests cannot see this at all.

    Previously the dependency was written the wrong way round — `albControllerPodIdentity.node.addDependency(albController)` — on the stated grounds that the SA is created by the chart, so waiting for it was "semantically clearer". But an association binds *by name* and does not require the SA to exist, so that edge was decorative while the ordering it inverted was mandatory. Because `cluster.addHelmChart(...)` lands in the **Cluster** stack and the association was created in the **Gateway** stack (which `addDependency(cluster)`), the chart was always created first: a deterministic failure on every fresh deploy, not a race that sometimes wins.

    The binding now lives where it can precede the chart: the IAM role in `IamStack` (alongside `podRole`), the association in `ClusterStack` (alongside `LiteLLMPodIdentity`), and `GatewayStack` only attaches `albController.node.addDependency(props.albControllerPodIdentity)` — same template, so it renders as a plain `DependsOn` with no cross-stack cycle. Verified on a fresh-region deploy: association created at `17:19:37Z`, controller pods at `17:23:34Z` with `restarts=0` and both env vars present, `created loadBalancer` at `17:24:17Z`, zero `NoCredentialProviders` hits — **no `rollout restart` anywhere**. Four tests in `test/snapshot/gateway-stack.test.ts` pin both directions, including the reverse red line (the association must *not* depend on the chart).

    ⚠️ **Upgrading an existing deployment is not in-place.** The association moves between stacks, so CloudFormation tries to create the new one while the old one still exists, and EKS rejects that: `ResourceInUseException: Association already exists` (verified against the live API). Delete the old association first, then deploy:

    ```bash
    CLUSTER=<your-cluster>            # e.g. LiteLLMGateway-eks
    ...
    npx cdk deploy --all
    # the controller pod predates the new association, so restart it once (this time only):
    kubectl -n kube-system rollout restart deploy/aws-load-balancer-controller
    ```

    Existing ALBs keep serving traffic throughout — the controller only needs credentials to reconcile *changes*. A fresh deploy needs none of this.
